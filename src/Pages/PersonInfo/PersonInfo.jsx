...
export function PersonInfo() {
  ...
  const { name, enName, photo, sex, age, death, birthday } = personData
      ...
      <div className={style.person}>
        <div className={style.photo}>
          {photo ? (
            ...
            <Empty description={'Нет фотографии'} />
          )}
        </div>
        ...
        <div className={style.personBlock}>
          ...
          <div className={style.personInfoBlock}>
            ...
            <p>
              День рождения:{' '}
              {`${personDateDays}.${personDateMount}.${personDateYear}г.`}
            </p>
          </div>
          <div></div>
        </div>
      </div>
      <h2 className={style.moviesBlock}>Участвовал в таких фильмах как:</h2>

      <div className={style.moviesContainerBlockInfo}>
        {moviesData.map((movie) => {
          return <MovieCard className={style.movieCards} data={movie} />
        })}
      </div>
    </Container>
  )
